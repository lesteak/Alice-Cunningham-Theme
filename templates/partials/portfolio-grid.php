      ...
      <div id="masonry-grid" class="flex flex-wrap">
        <?php foreach($posts as $post) { ?>
          ...
          <div class="w-1/2 md:w-1/3 masonry-item p-0.5 relative">
            <a href="<?php echo $thumb['url']; ?>" data-fancybox="<?php echo $post->post_name; ?>" data-caption="<?php echo $thumb['caption']; ?>" class="relative block">
          ...
          <div class="hidden">
            ...
            <?php foreach($gallery_images as $key => $gallery_image) { ?>
              <?php echo wp_get_attachment_image($gallery_image['id'], 'large', false, array('data-fancybox' => $post->post_name, 'data-caption' => $gallery_image['caption'])); ?> 
            <?php } ?>
          </div>
        <?php } ?>
      </div>
